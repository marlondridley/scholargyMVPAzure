...
const { OpenAIClient, AzureKeyCredential: OpenAIAzureKeyCredential } = require("@azure/openai");
...
const initializeClients = async () => {
    ...
    try {
        ...
        clients.openai = new OpenAIClient(
            `${config.openai.endpoint}/openai/deployments/${config.openai.chatModel}`,
            new OpenAIAzureKeyCredential(config.openai.apiKey)
        );

        // Initialize embedding client
        clients.embedding = new OpenAIClient(
            `${config.openai.endpoint}/openai/deployments/${config.openai.embeddingModel}`,
            new OpenAIAzureKeyCredential(config.openai.apiKey)
        );
...
const generateEmbedding = async (text, retries = 3) => {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await clients.embedding.getEmbeddings(
...
If the context doesn't contain relevant information, respond: "I don't have specific information about that in my current knowledge base, but I'd be happy to help with other college-related questions."`;
        ...
        const response = await clients.openai.getChatCompletions(
            messages,
            {
                maxTokens: 800,
                temperature: 0.3
...
router.post('/stream', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const stream = await clients.openai.getChatCompletions(
                messages,
                {
                    maxTokens: 800,
                    temperature: 0.3,
